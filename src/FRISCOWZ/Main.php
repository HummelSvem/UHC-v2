...
class Main extends PluginBase implements Listener {
	...
	public function alive(){
		foreach($this->getServer()->getOnlinePlayers() as $p){
			$p->sendMessage($this->prefix . "§7 Folgt uns auf Twitter :D");
		}
	}

	public function msg(){
		foreach($this->getServer()->getOnlinePlayers() as $p){
			$p->sendMessage($this->prefix . "Joint auf unseren Discord Server: https://discord.gg/CUvUU3F :)");
		}
	}
	...
	public function onJoin(PlayerJoinEvent $event){
		$event->getPlayer()->sendTip("§8[§bPlugin von zGodSky§8]\n\n\n\n\n\n\n\n\n");
		...
		if($sender instanceof Player and $sender->isOp()){
			switch($args[0]){
				case "reset":
					...
					$this->getServer()->broadcastMessage("§7[§bUHC§7] §7 UHC startet!");

					return true;
					break;
				case "help":
					$sender->sendMessage("§7<< §bUHC Edit By zGodSky §7>> ");
					$sender->sendMessage("§b/uhc reset: §7resettet UHC!");
					$sender->sendMessage("§b/uhc meetup: §7Kit Meetup");
					$sender->sendMessage("§b/uhc start: §7 startet UHC");
					$sender->sendMessage("§b/uhc tpall: §7Tpall ");
					$sender->sendMessage("§b/uhc meetop: §7kit meetup top!!");
					$sender->sendMessage("§b/uhc pvp <on/off>: §7PvP on/off!");
					$sender->sendMessage("§b/uhc scenario <hero>: §7Scenario!");
					$sender->sendMessage("§b/uhc globalmute: §7Chat off/on !");
				...
				case "pvp":
					if($args[1] === "on"){
						$this->pvp = true;
						$this->getServer()->broadcastMessage("§7[§bUHC§7] §7PvP aktiviert!");
					}
					if($args[1] === "off"){
						$this->pvp = false;
						$this->getServer()->broadcastMessage("§7[§bUHC§7] §7PvP deaktiviert");
					}
				...
				case "meetup":
					...
					$this->getServer()->broadcastMessage("§7[§bUHC§7-§3MEETUP§7] § startet §bMEETUP§7-§3Normal!");
				...
				case "start":
					...
					$this->getServer()->broadcastMessage("§7[§bUHC§7] §7UHC noch 10 Sekunden! §cNO Se Valla.");
					$this->getServer()->getScheduler()->scheduleDelayedTask(new class($this) extends PluginTask{
						/**
						 * @param int $currentTick
						 */
						public function onRun(int $currentTick){
							$this->getOwner()->getServer()->broadcastMessage("§7[§bUHC§7] §7UHC hat gestartet!");
						}
					}, 20 * 10);
					return true;
					break;
				case "meetop":
					foreach($this->getServer()->getOnlinePlayers() as $p){
						...
						$this->getServer()->broadcastMessage("§7[§bUHC§7-§3MEETUP§7] §7 du hast das §bMEETUP§7-§3OP!");
					}
				...
				case "globalmute":
					if($sender->hasPermission("UHC.host")){
						if($this->globalmute === false){
							$this->getServer()->broadcastMessage($this->prefix . TextFormat::GRAY . "Global Mute ist aktiviert!");
							$this->globalmute = true;

							return true;
						}else{
							$this->getServer()->broadcastMessage($this->prefix . TextFormat::GRAY . "Global Mute ist deaktiviert .");
				...
				case "tpall":
					foreach($this->getServer()->getOnlinePlayers() as $p){
						...
						$this->getServer()->broadcastMessage("§7[§bUHC§7]  §7Teleportieren...");
					}
			...
			$sender->sendMessage("§7[§bUHC§7] §cDu hast keine Rechte!");
		}
		return true;
	}
	...
	public function onPlayerDeath(PlayerDeathEvent $event){
		...
		if($cause instanceof EntityDamageByEntityEvent and $cause->getDamager() instanceof Player){
			...
			$killer->sendMessage("§7[§bUHC§7] §c hat 10 <3 §7gestorben ");
